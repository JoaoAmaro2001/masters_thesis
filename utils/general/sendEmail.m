...
%Source: https://undocumentedmatlab.com/articles/sending-html-emails-from-matlab
...
%   SENDMAIL(TO,SUBJECT,MESSAGE,ATTACHMENTS) sends an e-mail. TO is either
%   a character vector specifying a single address, or a cell array of
%   character vector addresses. SUBJECT is a character vector. MESSAGE is
%   either a character vector, cell array of character vectors, or string
%   array. If it is a character vector, the text will automatically wrap at
%   75 characters. If it is a cell/string array, it won't wrap, but each
%   cell starts a new line. In either case, use NEWLINE (char(10)) to
%   explicitly specify a new line. ATTACHMENTS is a character vector,cell
%   array of character vectors, or string array listing files to send along
%   with the message. Only TO and SUBJECT are required.
%
...
%   mail server is to look at the preferences of another e-mail application
%   or consult your administrator. If you cannot find out your server's
%   name, setting it to just 'mail' might work. If you do not set these
%   preferences SENDMAIL will try to determine them automatically by
%   reading environment variables and the Windows registry.
%
%   Example:
...
[to,subject,theMessage,attachments] = convertStringsToChars(to,subject,theMessage,attachments);
...
% Set subject.
if any(subject == newline) || any(subject == char(13))
...
if numel(attachments) == 0  && ~isHtml
    ...
    for iAttachments = 1:numel(attachments)
        ...
        % File name is UTF-8 because it is the most popular unicode encoding.
        filenameEncoding = "UTF-8";
        transferEncoding = "B";
        messageBodyPart.setFileName(MimeUtility.encodeText([fileName fileExt], filenameEncoding, transferEncoding));
...
elseif (length(dir(file)) == 1)
    % Absolute path.
    fullPathToFile = file;
elseif ~isempty(matlab.lang.internal.introspective.safeWhich(file))
    % A file on the path.
    fullPathToFile = matlab.lang.internal.introspective.safeWhich(file);
elseif ~isempty(matlab.lang.internal.introspective.safeWhich([file '.']))
...
if iscell(msgText)
    toSend = strjoin(reshape(msgText, 1, numel(msgText)),newline);
    return
end
...
lines = {};
maxLineLength = inf;
msgText = [newline msgText newline];
crList = find(msgText == newline);
...
%                 stop = start+maxLineLength-1;
        ...
        lines{1,end+1} = nextLine(start:stop); %#ok<AGROW> 
    end
end

toSend = strjoin(lines,newline);
